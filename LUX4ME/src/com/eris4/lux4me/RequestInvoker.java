...
public interface RequestInvoker {
	 ...
	 * create method of {@link com.eris4.lux4me.Lux4ClientFactory} used to intantiate this request invoker.
     *
	 * @throws com.eris4.lux4me.CannotConnectException if the communication cannot take place. This can happen
	...
	/**
	 * Closes the connection previously opened by the same thread that calls this method.
	 * Only if this is the last thread to disconnect, the connection with the server is actually closed.
	 */
	void disconnect();

	/**
	 * Closes the connection with the server even if not all threads which connected also diconnected.
	 * Standard usage is to call only in finalizer of the owner of the RequestInvoker.
	 */
